  ...
  <div class="report-view space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">日报、组合分析报告、Excel 导出</p>
      </div>
      <button class="btn-primary text-sm py-2 px-5">生成今日报告</button>
    </div>
    ...
    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <div v-for="report in reports" :key="report.id"
        ...
        <div class="flex items-start justify-between mb-3">
          <span :class="report.type === 'daily' ? 'badge-gold'
          ...
          <span class="flex items-center gap-1.5 text-xs"
            ...
            <span class="w-1.5 h-1.5 rounded-full animate-pulse"
                  :class="report.status === 'ready' ? 'bg-emerald-400' : 'bg-gold-400'"></span>
            {{ report.status === 'ready' ? '就绪' : '生成中' }}
          </span>
        </div>
        <h3 class="font-semibold mb-2 group-hover:text-gold-400 transition-colors">{{ report.title }}</h3>
        <div class="flex items-center justify-between text-xs text-gray-600">
          <span>{{ report.date }}</span>
          <div class="opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
            <button class="hover:text-cyan-400 transition-colors">预览</button>
            <button class="hover:text-emerald-400 transition-colors">下载</button>
          </div>
        </div>
      </div>
    </div>
