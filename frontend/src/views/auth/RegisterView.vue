  ...
  <div class="min-h-screen flex items-center justify-center -mt-16 px-4">
    <div class="w-full max-w-md">
      <div class="text-center mb-8">
        ...
        <p class="text-sm text-gray-500 mt-1">注册以使用 FUND-OS 全部功能</p>
      </div>
      ...
      <div class="card p-6 space-y-5">
        ...
        <div>
          ...
          <input v-model="form.username" type="text" placeholder="设置用户名" class="input-base" />
        </div>
        ...
        <div>
          ...
          <input v-model="form.confirmPassword" type="password" placeholder="再次输入密码" class="input-base"
                 @keyup.enter="handleRegister" />
        </div>

        <button :disabled="loading || !form.username || !form.email || !form.password"
                class="btn-primary w-full py-3" @click="handleRegister">
          {{ loading ? '注册中...' : '注册' }}
        </button>

        <p class="text-center text-sm text-gray-600">
          已有账号？
          <router-link to="/login" class="text-cyan-400 hover:text-cyan-300 transition-colors">立即登录</router-link>
        </p>
      </div>
    </div>
  </div>
